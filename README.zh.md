...
| NARI PCS-9611S | 模型发现（采用手动构建：该装置拒绝标准 `retrieveModel`）、分闸与合闸的**增强安全 SBO 操作**，以及在装置自身控制日志中得到确认的**符合 §20.8 的 Cancel**。已在把手处于**就地控制**的两个位置验证拒绝行为：装置拒绝命令但并不填写 `Loc` / `LocKey` / `LocSta`。完整读取模型并导出携带装置实际值的 CID |
