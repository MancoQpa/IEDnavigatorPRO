...
- خيار **قراءة كل القيم عند الاتصال** (مُعطَّل افتراضيًا): يجلب قيمة كل كائنات البيانات عند الاتصال، دون الحاجة إلى توسيع الشجرة أو تفعيل الاستقصاء الدوري.
...
- **الإلغاء (Cancel)** وفق IEC 61850-7-2 §20.8: يكرّر الأمر `Cancel` قيمة `ctlVal` الخاصة بأمر SELECT. جرى التحقق منه مقابل سجل التحكم في المُرحِّل نفسه.
- **فحص الشروط المسبق** قبل الأمر وبعده: `Beh` و`Health` وصلاحية التحكم (`Loc`، `LocKey`، `LocSta`) والتشابك (`CILO.EnaOpn` / `EnaCls`) وفحص التزامن (`RSYN.Rel`).
- عند الرفض يُعرض **ما أعاده الجهاز**: قيمة `AddCause` مع اسمها —أو `ServiceError` إذا لم ينشر الجهاز `AddCause`— إلى جانب `LastApplError`. وإذا أُرسل الأمر في **وضع الاختبار** إلى عقدة في الخدمة (`Beh=on`)، يُسمّى هذا السبب (*Blocked-by-Mode*) بدل نسبته إلى التحكم المحلي.
...
- النشر **مُعلَّم كمحاكاة افتراضيًا** (بت S في الترويسة وحقل `simulation` في الـ PDU): لا يقبله المشترك المطابق للمعيار إلا إذا كان هو نفسه في وضع المحاكاة (`LPHD.Sim`). ويمكن إلغاء التعليم من اللوحة، مع تنبيه في السجل.
- **مرشّح إعادة الإرسال**: يعرض تغيّرات الحالة فقط (`stNum` جديد) ويتجاهل رسائل النبض.
- يعرض جدول الرسائل **عنوان MAC المصدر** لكل إطار وما إذا كان يحمل **راية المحاكاة**.
...
- **توليد ملف CID من الجهاز المتصل**: تُقرأ جميع القيم قبل التصدير، بحيث يحمل الملف الإعداد الفعلي للجهاز. أما الكائنات التي لا يعيدها الجهاز فتُترك دون قيمة بدل كتابتها بقيمة افتراضية.
...
| NARI PCS-9611S | اكتشاف النموذج (عبر البناء اليدوي: الجهاز يرفض أمر `retrieveModel` القياسي)، و**مناورات SBO بأمان معزَّز** للفتح والإغلاق، و**الإلغاء (Cancel) وفق §20.8** مؤكَّدًا في سجل التحكم الخاص بالمرحّم نفسه. كما جرى التحقق من الرفض مع المفتاح في وضع **التحكم المحلي** في كلا الوضعين: الجهاز يرفض دون تعبئة `Loc` / `LocKey` / `LocSta`. قراءة كاملة للنموذج وتصدير ملف CID يحمل قيم الجهاز |
...
3. النقر بزر الفأرة الأيمن على `INSTALAR.bat` ← **تشغيل كمسؤول**. تُفتح نافذة تتضمن خياري **تثبيت** و**إلغاء التثبيت**؛ وإذا اكتشفت غياب Npcap فإنها تعرض زرًا يقوم بتنزيله وتثبيته.
4. لا يلزم Npcap إلا لـ GOOSE/SV في الطبقة الثانية. ويمكن تثبيته أيضًا بشكل منفصل من <https://npcap.com/#download>.
