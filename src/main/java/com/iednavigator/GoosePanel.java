 ...
 * Extracted from IEDNavigatorApp as part of Phase 5 refactoring.
 */
class GoosePanel {
    ...
    /** Package-visible: refresh the GoCBs table (called from IEDNavigatorApp after SCL load). */
    void refreshGooseControlBlocksPublic() {
        refreshGooseControlBlocks();
    }

    /** Package-visible wrapper for IEDNavigatorApp's SCL PARSING delegation. */
    ...
    // ─── SCL parsing (formerly in IEDNavigatorApp) ─────────────────────────────
    ...
    private void parseGoCBsFromScl(File sclFile) {
        ...
        try {
            ...
            if (iedNodes.getLength() > 0) {
                ...
                iedName = iedEl.getAttribute("name");
            ...
            // Solo GSEControl del primer IED (un SCD multi-IED contiene GoCBs ajenos)
            ...
            // Communication/GSE solo de los ConnectedAP del primer IED
            ...
            for (int c = 0; c < connectedAPs.getLength(); c++) {
                ...
                if (!iedName.isEmpty() && !iedName.equals(connAP.getAttribute("iedName"))) continue;
                ...
                for (int i = 0; i < gseNodes.getLength(); i++) {
                    ...
                    gseInfo.put(key, SclFileProcessor.parseGseElement(gse));
                }
            }
            ...
            for (int i = 0; i < gseControls.getLength(); i++) {
                ...
                if (gseInfo.containsKey(key)) {
                    SclFileProcessor.applyGseInfo(gcb, gseInfo.get(key));
                }
    ...
    private void parseGoCBsFromScl(File sclFile, int iedIndex) {
        ...
        // (IEDNavigatorApp keeps sclReports; we pass datasets back)
    }
    ...
    // ─── Publisher sync (F21: moved from IEDNavigatorApp.java) ────────────────
    ...
    // ─── FCDA helpers (used by GOOSE-MODEL SYNC in IEDNavigatorApp) ───────────
