...
public class GoosePublisher {
    ...
    private int vlanPriority = 4; // prioridad por defecto GOOSE según IEC 61850-8-1
    ...
    private void publishGoose() throws Exception {
        ...
        if (sendHandle != null && sendHandle.isOpen()) {
            ...
            if (vlanId >= 0) {
                // Etiqueta 802.1Q con VLAN-ID/VLAN-PRIORITY del SCL
                Dot1qVlanTagPacket.Builder vlanBuilder = new Dot1qVlanTagPacket.Builder();
                vlanBuilder.priority((byte) vlanPriority)
                           .cfi(false)
                           .vid((short) vlanId)
                           .type(GOOSE_ETHERTYPE)
                           .payloadBuilder(new UnknownPacket.Builder().rawData(payload));
                ethBuilder.type(EtherType.DOT1Q_VLAN_TAGGED_FRAMES)
                          .payloadBuilder(vlanBuilder);
            } else {
                ethBuilder.type(GOOSE_ETHERTYPE)
                          .payloadBuilder(new UnknownPacket.Builder().rawData(payload));
            }
    ...
    public void setVlan(int vlanId, int vlanPriority) {
        this.vlanId = vlanId;
        if (vlanPriority >= 0 && vlanPriority <= 7) this.vlanPriority = vlanPriority;
    }
