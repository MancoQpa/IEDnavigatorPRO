 ...
 * Extraído de IEDNavigatorApp.java — Fase 6 de refactorización.
 *
...
class ReportsPanel {
    ...
    JPanel createPanel() {
        ...
        reportsTable.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (row < 0) return;
            String ref = (String) reportsTableModel.getValueAt(row, 0); // "LD/LN.rcbName"
            if (ref == null || ref.isEmpty()) return;
            // En modo cliente los RCB no aparecen como nodos del árbol → navegar al LN contenedor
            int dot = ref.indexOf('.');
            onNavigate.accept(dot > 0 ? ref.substring(0, dot) : ref);
        });
        ...
        reportDataTable.getSelectionModel().addListSelectionListener(e -> {
            if (e.getValueIsAdjusting() || onNavigate == null) return;
            int row = reportDataTable.getSelectedRow();
            if (row < 0) return;
            String ref = (String) reportDataTableModel.getValueAt(row, 3); // columna "Referencia"
