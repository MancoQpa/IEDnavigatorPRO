    ...
    JPanel createPanel() {
        ...
        settingGroupsTable.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                // Navegar al nodo seleccionado en el árbol del modelo (como en Dataset)
        ...
        sgValuesTable.getSelectionModel().addListSelectionListener(e -> {
            if (e.getValueIsAdjusting() || onNavigate == null) return;
            int vRow = sgValuesTable.getSelectedRow();
            int sgRow = settingGroupsTable.getSelectedRow();
            if (vRow < 0 || sgRow < 0) return;
            String baseRef = (String) settingGroupsTableModel.getValueAt(sgRow, 4); // IEDLD/LN.DO
            String attr = (String) sgValuesTableModel.getValueAt(vRow, 0);          // DO.sub.attr
            if (baseRef == null || attr == null) return;
            int dot = attr.indexOf('.');
            String target = dot > 0 ? baseRef + attr.substring(dot) : baseRef;
