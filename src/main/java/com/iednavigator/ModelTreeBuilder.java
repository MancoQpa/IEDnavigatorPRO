 ...
 * Extraído de IEDNavigatorApp.java — Fase 3 de refactorización.
 *
...
class ModelTreeBuilder {
    ...
    private static void addGoCBsToLdNode(
        ...
        for (SclGoCB gcb : ldGocbs) {
            ...
            if (gcb.vlanId >= 0)       addGocbProperty(gcbNode, "VLAN-ID", String.format("0x%03X (%d)", gcb.vlanId, gcb.vlanId));
            if (gcb.vlanPriority >= 0) addGocbProperty(gcbNode, "VLAN-PRIORITY", String.valueOf(gcb.vlanPriority));
            if (gcb.minTime >= 0)      addGocbProperty(gcbNode, "MinTime", gcb.minTime + " ms");
            if (gcb.maxTime >= 0)      addGocbProperty(gcbNode, "MaxTime", gcb.maxTime + " ms");
    ...
    // ─── Recursive tree update (F23: moved from IEDNavigatorApp.java) ─────────
    ...
    // ─── FCDA tree navigation (F22: moved from IEDNavigatorApp.java) ──────────
